  ...
  return (
    <>
      <div
        ...
        className={`relative flex h-[440px] flex-col rounded-xl border p-6 neon-card ${
          pattern.disabled ? "cursor-not-allowed opacity-40 grayscale" : `cursor-pointer ${borderClass[pattern.buildStatus]}`
        }`}
      >
        ...
        {pattern.disabled ? (
          <button
            disabled
            className="flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-medium border border-gray-700/40 bg-gray-800/40 text-slate-600 cursor-not-allowed"
          >
            Not Available
          </button>
        ) : active ? (

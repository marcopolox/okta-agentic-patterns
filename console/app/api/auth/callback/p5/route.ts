...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(`/patterns/p5?error=${encodeURIComponent(error)}`, base)
    );
  }

  if (!code) {
    return NextResponse.redirect(new URL("/patterns/p5?error=no_code", base));
  }

  const domain = process.env.OKTA_DOMAIN;
  const clientId = process.env.P5_OKTA_CLIENT_ID;
  const clientSecret = process.env.P5_OKTA_CLIENT_SECRET;
  const redirectUri = `${base}/api/auth/callback/p5`;

  if (!domain || !clientId || !clientSecret) {
    return NextResponse.redirect(
      new URL("/patterns/p5?error=missing_config", base)
    );
  }
  ...
  if (!tokenRes.ok) {
    ...
    return NextResponse.redirect(
      new URL(`/patterns/p5?error=${encodeURIComponent(body)}`, base)
    );
  }

  const tokens = await tokenRes.json() as { id_token?: string };
  const response = NextResponse.redirect(new URL("/patterns/p5", base));

  if (tokens.id_token) {
    response.cookies.set("p5_id_token", tokens.id_token, {
      httpOnly: true,
      maxAge: 3600,
      sameSite: "lax",
      path: "/",

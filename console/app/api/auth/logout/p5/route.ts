...
export async function GET(req: NextRequest) {
  ...
  const idToken = req.cookies.get("p5_id_token")?.value;
  const domain = process.env.OKTA_DOMAIN;

  const postLogoutUri = new URL("/patterns/p5", base).toString();
  ...
  await fetch(`${eventBusUrl}/events/p5`, { method: "DELETE" }).catch(() => {});

  const response = NextResponse.redirect(redirectTarget);
  response.cookies.delete("p5_id_token");
  return response;
}

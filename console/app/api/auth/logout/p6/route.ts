...
export async function GET(req: NextRequest) {
  ...
  const idToken = req.cookies.get("p6_id_token")?.value;
  const domain = process.env.OKTA_DOMAIN;

  const postLogoutUri = new URL("/patterns/p6", base).toString();
  ...
  await fetch(`${eventBusUrl}/events/p6`, { method: "DELETE" }).catch(() => {});

  const response = NextResponse.redirect(redirectTarget);
  response.cookies.delete("p6_id_token");
  return response;
}

...
export async function GET(req: NextRequest) {
  ...
  const idToken = req.cookies.get("p3_id_token")?.value;
  const domain = process.env.OKTA_DOMAIN;

  const postLogoutUri = new URL("/patterns/p3", base).toString();
  ...
  await fetch(`${eventBusUrl}/events/p3`, { method: "DELETE" }).catch(() => {});

  const response = NextResponse.redirect(redirectTarget);
  response.cookies.delete("p3_id_token");
  return response;
}
